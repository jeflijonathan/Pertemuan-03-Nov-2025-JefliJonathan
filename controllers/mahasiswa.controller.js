import mahasiswaSchema from "../models/mahasiswa.js";
import bcrypt from "bcryptjs";

const createMahasiswa = async (req, res) => {
  try {
    const { fullname, npm, email, date_of_born, password } = req.body;
    const existsMahasiswa = await mahasiswaSchema.findOne({ npm });
    if (existsMahasiswa)
      return res.status(400).json({ message: "NPM already registered" });

    const passwordHash = await bcrypt.hash(password, 10);
    const newMahasiswa = await mahasiswaSchema.create({
      fullname,
      npm,
      email,
      date_of_born,
...
const loginMahasiswa = async (req, res) => {
  try {
    ...
    const mahasiswa = await mahasiswaSchema.findOne({ npm });
    if (!npm) {
      console.log("kosong npmnya");
    }
    if (!mahasiswa)
      return res.status(401).json({ message: "Invalid NPM or password" });
...
const allMahasiswa = async (req, res) => {
  try {
    const mahasiswa = await mahasiswaSchema.find().select("-password");
    if (!mahasiswa)
      return res.status(404).json({ message: "No mahasiswa found" });
    res.status(200).json({
...
const updateMahasiswa = async (req, res) => {
  try {
    ...
    const updatedMahasiswa = await mahasiswaSchema.findByIdAndUpdate(
      id,
      { fullname, npm, email, date_of_born, password, isActive },
      { new: true }
    );
    if (!updatedMahasiswa)
...
const deleteMahasiswa = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedMahasiswa = await mahasiswaSchema.findByIdAndDelete(id);
    if (!deletedMahasiswa)
      return res.status(404).json({ message: "Mahasiswa not found" });
...
const getMahasiswaById = async (req, res) => {
  try {
    const { id } = req.params;
    const mahasiswa = await mahasiswaSchema.findById(id);
    if (!mahasiswa)
      return res.status(404).json({ message: "Mahasiswa not found" });
    const mahasiswaObj = mahasiswa.toObject();
    delete mahasiswaObj.password;
